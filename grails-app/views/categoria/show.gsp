	...
	<body>
		...
		<div id="show-categoria" class="content scaffold-show" role="main">
			...
			<ol class="property-list categoria">
				...
				<g:if test="${categoriaInstance?.producto}">
				<li class="fieldcontain">
					<span id="producto-label" class="property-label"><g:message code="categoria.producto.label" default="Producto" /></span>
					
						<g:each in="${categoriaInstance.producto}" var="p">
						<span class="property-value" aria-labelledby="producto-label"><g:link controller="producto" action="show" id="${p.id}">${p?.encodeAsHTML()}</g:link></span>
						</g:each>
